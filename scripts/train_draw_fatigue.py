...
MODEL_PATH = MODEL_DIR + "/baseline_draw_fatigue.pkl"
...
def load_data():
    ...
    df = pd.read_sql(
        ...
        SELECT
            ...
            home_has_future_match,
            away_has_future_match
        FROM train_baseline_draw_fatigue
        """,
        conn
    )
    conn.close()
...
def main():
    ...
    pipeline = Pipeline([
        ...
        ("clf", LogisticRegression(
            solver="lbfgs",
            max_iter=1200,
            class_weight={"H": 1.0, "D": 1.5, "A": 1.0}
        ))
    ])
